...
def _load_model(checkpoint: dict[str, Any], args: DictConfig, device: torch.device) -> tuple[Any, Any]:
    role = str(checkpoint["experiment_role"])
    if role == "frozen_prompt_FP5":
        bundle = load_trainable_sketch_hidden_encoder(
            model_name=str(args.model_name), pretrained=args.pretrained,
            device=device, mode="partial", unfreeze_depth=4,
        )
        ...
        model = FrozenPromptModel(
            ...
            train_visual_layernorm=role == "frozen_prompt_FP_LN",
        ).to(device)
...
def run(args: DictConfig) -> None:
    ...
    if not isinstance(checkpoint, dict) or checkpoint.get("campaign") != CAMPAIGN:
        raise ValueError("checkpoint is not from the frozen-prompt campaign")
    ...
    loader_options = {
        "batch_size": int(args.eval_batch_size), "shuffle": False,
        "num_workers": int(args.num_workers), "pin_memory": bool(args.pin_memory),
    }
    sketch_loader = DataLoader(RetrievalEvalDataset(sketches, transform), **loader_options)
    ...
    gallery_model = _FrozenEncoderAdapter(model.encoder) if str(checkpoint["experiment_role"]) == "frozen_prompt_FP5" else model
    gallery = encode_prompted_loader(gallery_model, photo_loader, photo=True)
    evaluation = evaluate_prompted(queries, gallery, query_chunk_size=int(args.query_chunk_size), device=device)
    result = {
        "campaign": CAMPAIGN,
...
@hydra.main(version_base="1.3", config_path=HYDRA_CONFIG_DIR, config_name="evaluate_frozen_prompt")
