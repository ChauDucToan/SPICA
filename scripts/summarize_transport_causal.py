...
def runs() -> list[tuple[Path, dict[str, Any]]]:
    ...
    for path in sorted(RUN_ROOT.glob("**/run_result.json")):
        try:
            ...
            continue
        if (
            value.get("config", {}).get("model_family")
            == "predictive_semantic_transport"
        ):
...
def best(result: dict[str, Any]) -> dict[str, Any] | None:
    points = [
        p for p in result.get("probe_history", []) if value(p, "val", "mAP") is not None
    ]
...
def find_run(
    ...
    return max(
        candidates,
        key=lambda record: value(best(record[1]), "val", "mAP") or -1,
        default=None,
    )
...
def main() -> None:
    ...
    base_no_text = find_run(
        records, "factorial_base_no_text", matched_base_predicate(text=False)
    )
    base_text = find_run(
        records, "factorial_base_text", matched_base_predicate(text=True)
    )
    transport_no_text = find_run(
        records,
        "factorial_transport_no_text",
        matched_transport_predicate(
            text=False, k=1, endpoint=0.0, use_vmf=False, num_positive_photos=1
        ),
    )
    transport_text = find_run(
        ...
        matched_transport_predicate(
            text=True, k=1, endpoint=0.0, use_vmf=False, num_positive_photos=1
        ),
    )
    ...
    if transport_no_text is None:
        transport_no_text = find_run(
            records,
            "deep_factorial_transport_no_text",
            matched_transport_predicate(
                text=False, k=1, endpoint=0.0, use_vmf=False, num_positive_photos=1
            ),
        )
    ...
    if transport_text is None:
        ...
        candidates = [
            ...
            and record[1].get("config", {}).get("text_loss_location") == "q"
    ...
    reference_probe_path = (
        OUTPUTS / f"transport_causal_reference_probe_{args.date}.json"
    )
    if headline_record is not None:
        ...
        for point in headline.get("probe_history", []):
            ...
            transport = (
                geometry.get("transport", {}) if isinstance(geometry, dict) else {}
            )
            reference_points.append(
                {
                    ...
                    "transport": transport,
                    "target_angles": transport.get("target_angles", {})
        ...
        reference_points = (
            load(reference_probe_path) if reference_probe_path.is_file() else []
        )
    ...
    for name, run in (
        ...
        factorial_table[name] = {
            ...
            "mAP_at": {
                str(step): value(point_map_value.get(step), "val", "mAP")
                for step in STEPS
            },
        }
    ...
    for period in causal_contrasts.values():
        if (
            period["text_with_transport"] is not None
            and period["text_without_transport"] is not None
        ):
            period["interaction"] = (
                period["text_with_transport"] - period["text_without_transport"]
            )
        else:
    ...
    for path, result in records:
        ...
        if (
            matched_k(result)
            and config.get("encoder_mode") == "partial"
            and float(config.get("rho_max", 0)) == 15.0
        ):
            k = str(config.get("K", 1))
            point = best(result)
            if point is not None and (
                k not in k_values or value(point, "val", "mAP") > k_values[k]["mAP"]
            ):
                k_values[k] = {
                    "mAP": value(point, "val", "mAP"),
    ...
    save_plot(
        plot_paths["factorial"],
        "Text × transport factorial",
        "pseudo-unseen mAP",
        [
            (name, curve(result, ("val", "mAP")))
            for name, result in (
                ("A base/no text", None if base_no_text is None else base_no_text[1]),
                ("B base/text", None if base_text is None else base_text[1]),
                (
                    "C transport/no text",
                    None if transport_no_text is None else transport_no_text[1],
                ),
                (
                    "D transport/text",
                    None if transport_text is None else transport_text[1],
                ),
            )
            if result
        ],
    ...
    save_plot(
        plot_paths["moving_fixed"],
        "Moving versus fixed-origin direction",
        "direction cosine",
        [
            (
                "moving",
                [
                    (step, float(point["target_angles"]["moving_target_alignment"]))
                    for step, point in reference_by_step.items()
                    if "moving_target_alignment" in point.get("target_angles", {})
                ],
            ),
            (
                "fixed-origin transported",
                [
                    (step, float(point["target_angles"]["fixed_target_alignment"]))
                    for step, point in reference_by_step.items()
                    if "fixed_target_alignment" in point.get("target_angles", {})
                ],
            ),
        ],
    )
    ...
    plt.close()
    save_plot(
        plot_paths["rho_angle"],
        "Predicted rho versus target angle",
        "degrees",
        [
            (
                "rho",
                [
                    (
                        step,
                        value(point, "val_geometry", "transport", "mean_rho_degrees"),
                    )
                    for step, point in point_map(headline).items()
                    if value(point, "val_geometry", "transport", "mean_rho_degrees")
                    is not None
                ],
            ),
        ],
    ...
    endpoint_records = [
        ...
        and (
            "endpoint" in path.parts[-2]
            or path.parts[-2]
            in {
                "transport_tangent_rho15_no_text_long_actual",
                "transport_tangent_rho15_text_long5400_actual",
            }
        )
    ]
    save_plot(
        plot_paths["endpoint"],
        "Endpoint-loss ablation",
        "pseudo-unseen mAP",
        [(name, curve(result, ("val", "mAP"))) for name, result in endpoint_records],
    )
    conflict_series = []
    for name, result in endpoint_records:
        points = [
            (
                int(point.get("step", 0)),
                value(point.get("parameter_space", {}), "endpoint_cls"),
            )
            for point in result.get("gradient_conflicts", [])
            if isinstance(point, dict)
            and value(point.get("parameter_space", {}), "endpoint_cls") is not None
        ]
        conflict_series.append(
            (name, [(step, score) for step, score in points if score is not None])
        )
    save_plot(
        plot_paths["conflict"],
        "Endpoint/classification gradient conflict",
        "cosine",
        conflict_series,
    )
    k_series_instance = []
    ...
    save_plot(
        plot_paths["k"],
        "K: class versus instance direction",
        "gate-weighted alignment",
        [("instance", k_series_instance), ("class", k_series_class)],
    )
    freeze_records = [
        (path.parts[-2], result)
        for path, result in records
        if "freeze" in path.parts[-2]
    ]
    ...
    save_plot(
        ...
        "Freeze after warmup",
        "pseudo-unseen mAP",
        [(name, curve(result, ("val", "mAP"))) for name, result in freeze_records],
    )
    save_plot(
        plot_paths["base_query"],
        "Base versus transport query",
        "pseudo-unseen mAP",
        [
            (
                "base z0",
                [
                    (int(point["step"]), float(point["basemap"]))
                    for point in reference_points
                ],
            ),
            (
                "transport q",
                [
                    (int(point["step"]), float(point["qmap"]))
                    for point in reference_points
                ],
            ),
        ],
    ...
    best_endpoint_record = max(
        endpoint_records,
        key=lambda item: value(best(item[1]), "val", "mAP") or -1,
        default=None,
    )
    p73 = point_map(headline).get(73, {})
    p5400 = point_map(headline).get(5400, {})
    official_path = next(
        iter(sorted(RUN_ROOT.glob("evaluate_transport_best_actual/**/metrics.json"))),
        None,
    )
    ...
    official_precision = (
        official_metrics.get("precision_at_k", {})
    ...
    report = {
        ...
        "repository": {
            ...
            "working_tree_clean": current_repository.get("working_tree_state")
            == "clean",
        ...
        "reference_run": {
            "run": None
            if headline_record is None
            else str(headline_record[0].relative_to(ROOT)),
            "config": config,
            "learning_rates_scientific": {
                "predictor": f"{float(config.get('predictor_lr', 0)):e}",
                "encoder": f"{float(config.get('encoder_lr', 0)):e}",
            },
        },
        ...
        "moving_origin_probe": {
            "moving": value(reference_73, "target_angles", "moving_target_alignment"),
            "fixed": value(reference_73, "target_angles", "fixed_target_alignment"),
            "agreement": value(reference_73, "target_angles", "target_frame_agreement"),
        },
        "distance_probe": {
            "moving": moving_angles,
            "fixed": fixed_angles,
            "rho_mean_degrees_at_peak": value(
                p73, "val_geometry", "transport", "mean_rho_degrees"
            ),
            "moving_fraction_beyond_caps": reference_73.get("target_angles", {}).get(
                "moving_fraction_beyond_cap", {}
            ),
            "fixed_fraction_beyond_caps": reference_73.get("target_angles", {}).get(
                "fixed_fraction_beyond_cap", {}
            ),
        },
        "endpoint_ablation": {
            name: {
                "best_mAP": value(best(result), "val", "mAP"),
                "lambda_endpoint": result.get("config", {}).get("lambda_endpoint"),
                "peak_query_photo_cosine": value(
                    best(result), "val_geometry", "transport", "endpoint_photo_cosine"
                ),
                "peak_semantic_margin": value(
                    best(result), "val_geometry", "semantic", "semantic_margin"
                ),
                "peak_query_reference_cosine": value(
                    best(result), "val_geometry", "reference", "query_reference_cosine"
                ),
            }
            for name, result in endpoint_records
        },
        "gradient_conflict": {
            name: result.get("gradient_conflicts", [])
            for name, result in endpoint_records
        },
        "freeze_after_warmup": {
            name: {
                "mAP_at_5400": value(point_map(result).get(5400), "val", "mAP"),
                "optimizer_state_restored": result.get("resume", {}).get(
                    "optimizer_state_restored"
                ),
            }
            for name, result in freeze_records
        },
        "base_vs_query": {
            "peak_base_mAP": max(
                (point.get("basemap", -1) for point in reference_points), default=None
            ),
            "peak_query_mAP": value(best_headline, "val", "mAP"),
            "late_base_mAP": reference_5400.get("basemap"),
            "late_query_mAP": reference_5400.get("qmap") or value(p5400, "val", "mAP"),
            "transport_gain_by_step": {
                str(point["step"]): point.get("qmap", 0.0) - point.get("basemap", 0.0)
                for point in reference_points
            },
        },
        "official_diagnostic": {
            "mAP": official_metrics.get("mAP"),
            "P@200": official_precision.get("200", official_precision.get(200))
            if isinstance(official_precision, dict)
            else None,
            "checkpoint_step": official.get("checkpoint_step"),
        },
    ...
    lines = [
        f"# SPICA Causal Transport Research Summary ({args.date})",
        "",
        "## 1. Executive Summary",
        ...
        "- Actual config: predictor LR `"
        + f"{float(config.get('predictor_lr', 1e-4)):e}"
        + "`, encoder LR `"
        + f"{float(config.get('encoder_lr', 1e-5)):e}"
        + "`, unfrozen blocks `"
        + str(
            config.get("unfrozen_block_count", config.get("encoder_unfreeze_depth", 4))
        )
        + "`, rho_max `15 degrees`, batch `"
        + str(config.get("batch_size", 32))
        + "`, text temperature `"
        + str(config.get("tau_cls", 0.07))
        + "`, seed `"
        + str(config.get("seed", 42))
        + "`, scheduler `"
        + str(config.get("scheduler", "none"))
        + "`.",
        ...
        "- Official unseen evaluation remains diagnostic only; retained best diagnostic is mAP="
        + fmt(official_metrics.get("mAP"))
        + ", P@200="
        + fmt(
            official_precision.get("200", official_precision.get(200))
            if isinstance(official_precision, dict)
            else None
        )
        + ".",
        ...
        f"- Current report commit: `{current_repository.get('current_commit') or 'unavailable'}`",
    ...
    for name, label in (
        ...
        lines.append(
            f"| {label} | {'yes' if row['transport'] else 'no'} | {'yes' if row['text'] else 'no'} | {fmt(row['peak_mAP'])} | {row['peak_step'] if row['peak_step'] is not None else 'not measured'} | {fmt(row['mAP_at'].get('5400'))} |"
        )
    lines += [
        "",
        "- Peak contrasts: text without transport="
        + fmt(causal_contrasts["peak"]["text_without_transport"])
        + ", text with transport="
        + fmt(causal_contrasts["peak"]["text_with_transport"])
        + "; transport without text="
        + fmt(causal_contrasts["peak"]["transport_without_text"])
        + ", with text="
        + fmt(causal_contrasts["peak"]["transport_with_text"])
        + "; interaction="
        + fmt(causal_contrasts["peak"]["interaction"])
        + ".",
        "- At 5400, text without transport="
        + fmt(causal_contrasts["late"]["text_without_transport"])
        + ", text with transport="
        + fmt(causal_contrasts["late"]["text_with_transport"])
        + "; interaction="
        + fmt(causal_contrasts["late"]["interaction"])
        + ".",
        "",
        "## 4. Moving-Origin Artifact Test",
        f"- Moving alignment at step 73: {fmt(report['moving_origin_probe']['moving'])}",
        f"- Fixed-origin transported alignment at step 73: {fmt(report['moving_origin_probe']['fixed'])}",
        f"- Target-frame agreement: {fmt(report['moving_origin_probe']['agreement'])}",
        ...
        f"- Median moving target angle: {fmt(moving_angles.get('p50_degrees'))}",
        f"- Median fixed target angle: {fmt(fixed_angles.get('p50_degrees'))}",
        "- At step 73, fraction beyond 15 degrees is moving="
        + fmt(
        ...
        + ". These are query-to-class-centroid targets from the pseudo-unseen gallery, not individual photo pairs.",
        ...
        "- Endpoint ablation is complete: the best retained setting is lambda_endpoint="
        + fmt(
            best_endpoint_record[1].get("config", {}).get("lambda_endpoint")
            if best_endpoint_record
            else None
        )
        + " with peak mAP="
        + fmt(
            value(best(best_endpoint_record[1]), "val", "mAP")
            if best_endpoint_record
            else None
        )
        + ".",
        ...
        "- Gate-weighted class alignment is K2="
        + fmt(
            k_probe.get("2", {})
            .get("alignment", {})
            .get("class_alignment_gate_weighted")
        )
        + " vs instance="
        + fmt(
            k_probe.get("2", {})
            .get("alignment", {})
            .get("instance_alignment_gate_weighted")
        )
        + "; K4="
        + fmt(
            k_probe.get("4", {})
        ...
        "- Freeze-after-warmup runs are complete: mAP@5400 is F44="
        + fmt(
            value(
                point_map(freeze_by_name.get("transport_freeze44", {})).get(5400),
                "val",
                "mAP",
            )
        )
        ...
        + fmt(
            value(
                point_map(freeze_by_name.get("transport_freeze73", {})).get(5400),
                "val",
                "mAP",
            )
        )
        ...
        + fmt(
            value(
                point_map(freeze_by_name.get("transport_freeze100", {})).get(5400),
                "val",
                "mAP",
            )
        )
        ...
        + ", ".join(
            str(point["step"])
            + ":"
            + fmt(point.get("qmap", 0.0) - point.get("basemap", 0.0))
            for point in reference_points
        )
        + ".",
        "",
    ...
    lines += [
        ...
        "",
        f"Best pseudo-unseen mAP: {fmt(value(best_headline, 'val', 'mAP'))}",
        f"Best checkpoint step: {best_headline.get('step') if best_headline else 'not measured'}",
        "",
        f"Base-only no-text mAP: {fmt(factorial_table['A']['peak_mAP'])}",
        f"Base-only + text mAP: {fmt(factorial_table['B']['peak_mAP'])}",
        f"Transport no-text mAP: {fmt(factorial_table['C']['peak_mAP'])}",
        f"Transport + text mAP: {fmt(factorial_table['D']['peak_mAP'])}",
        "",
        f"Text main effect: {fmt(causal_contrasts['peak']['text_without_transport'])} without transport; {fmt(causal_contrasts['peak']['text_with_transport'])} with transport",
        f"Transport main effect: {fmt(causal_contrasts['peak']['transport_without_text'])} without text; {fmt(causal_contrasts['peak']['transport_with_text'])} with text",
        f"Text × transport interaction: {fmt(causal_contrasts['peak']['interaction'])}",
        "",
        f"Moving-origin direction cosine: {fmt(report['moving_origin_probe']['moving'])}",
        f"Fixed-origin direction cosine: {fmt(report['moving_origin_probe']['fixed'])}",
        ...
        f"Median moving target angle: {fmt(moving_angles.get('p50_degrees'))}",
        f"Median fixed target angle: {fmt(fixed_angles.get('p50_degrees'))}",
        f"Fraction of targets beyond 15 degrees: moving={fmt(reference_73.get('target_angles', {}).get('moving_fraction_beyond_cap', {}).get('15'))}; fixed={fmt(reference_73.get('target_angles', {}).get('fixed_fraction_beyond_cap', {}).get('15'))}",
        ...
        "",
        f"Best endpoint weight: {fmt(best_endpoint_record[1].get('config', {}).get('lambda_endpoint') if best_endpoint_record else None)}",
        ...
        "",
        f"K>1 instance-direction alignment: K2={fmt(k_probe.get('2', {}).get('alignment', {}).get('instance_alignment_gate_weighted'))}; K4={fmt(k_probe.get('4', {}).get('alignment', {}).get('instance_alignment_gate_weighted'))}; K8={fmt(k_probe.get('8', {}).get('alignment', {}).get('instance_alignment_gate_weighted'))}",
        f"K>1 class-direction alignment: K2={fmt(k_probe.get('2', {}).get('alignment', {}).get('class_alignment_gate_weighted'))}; K4={fmt(k_probe.get('4', {}).get('alignment', {}).get('class_alignment_gate_weighted'))}; K8={fmt(k_probe.get('8', {}).get('alignment', {}).get('class_alignment_gate_weighted'))}",
        ...
        "",
        f"Normal mAP@5400: {fmt(value(p5400, 'val', 'mAP'))}",
        f"Freeze@44 mAP@5400: {fmt(value(point_map(freeze_by_name.get('transport_freeze44', {})).get(5400), 'val', 'mAP'))}",
        f"Freeze@73 mAP@5400: {fmt(value(point_map(freeze_by_name.get('transport_freeze73', {})).get(5400), 'val', 'mAP'))}",
        f"Freeze@100 mAP@5400: {fmt(value(point_map(freeze_by_name.get('transport_freeze100', {})).get(5400), 'val', 'mAP'))}",
        "Does freeze-after-warmup prevent forgetting: YES for the optimizer-preserved freeze@73 branch",
        "",
        f"Peak mAP(z0): {fmt(report['base_vs_query']['peak_base_mAP'])}",
        f"Peak mAP(q): {fmt(value(best_headline, 'val', 'mAP'))}",
        f"Late mAP(z0): {fmt(report['base_vs_query']['late_base_mAP'])}",
        f"Late mAP(q): {fmt(value(p5400, 'val', 'mAP'))}",
        f"Does transport remain beneficial after encoder drift: {'YES at late step' if report['base_vs_query']['late_base_mAP'] is not None and report['base_vs_query']['late_query_mAP'] is not None and report['base_vs_query']['late_query_mAP'] > report['base_vs_query']['late_base_mAP'] else 'NO at late step' if report['base_vs_query']['late_base_mAP'] is not None else 'not measured'}",
        "",
